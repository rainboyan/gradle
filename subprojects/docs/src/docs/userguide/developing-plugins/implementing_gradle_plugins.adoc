...
In projects that use plugins as dependencies, it is possible to request the variants of plugin dependencies that support a different Gradle version. This allows a
multi-variant plugin depending on other plugins to access their APIs which are exclusively provided in their version-specific variants.
